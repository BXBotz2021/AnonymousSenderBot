...
async def _calls(anonbot, callback_query):
    ...
    if callback_query.data.lower() == "remove":
        ...
        await anonbot.edit_message_caption(
            chat_id=chat_id, message.id=message.id, caption=caption, reply_markup=InlineKeyboardMarkup([Data.add_button])
        )
    if callback_query.data.lower() == "add":
        caption = callback_query.message.reply_to_message.caption
        if caption:
            await anonbot.edit_message_caption(
                chat_id=chat_id, message.id=message.id, caption=caption, reply_markup=InlineKeyboardMarkup([Data.remove_button])
            )
        else:
